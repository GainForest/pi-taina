...
| `CGS_SERVICE_URL` | For shared org creation | Internal shared-organization service URL. Users only see this as “organizations”. |
| `CGS_SERVICE_DID` | For shared org creation | Internal service DID used to authenticate organization writes. |
| `CGS_GROUP_PDS_URL` | No | PDS used for reading organization records (default: `https://gainforest.id`) |
| `GAINFOREST_INVITE_CODE` | Legacy only | Invite code for older standalone organization-account creation flows |
